...
벤더 스크립트가 **지우기 직전에 사람의 확인을 받도록** 되어 있다. 그 확인을
건너뛰게 만드는 것은 이 스킬이 하지 않는다 — 플래시를 지우고 쓰는 단계라
사람이 판단을 소유해야 한다. 요청받아도 그 구성을 만들거나 안내하지 않는다.

대신 앞뒤를 자동화했다. **실제로 줄어드는 수고는 거기에 있다.**

```
라이팅 전   run --mode check    보드가 붙나 / 지금 뭐가 올라가 있나   5초
라이팅      사람이 GUI 에서      run --mode guide 가 단계를 짚어줌
라이팅 후   run --mode check    새로 써진 게 맞나                    5초
```

HE1i 작업에서 오래 걸린 곳은 클릭이 아니라 **앞뒤 확인**이었다 — 버튼을 못 찾고,
`Image&Hsm` 을 안 누르고, `SYSDOWN` 을 실패로 오해하고, CAN 이 안 나와 헤맸다.
그 지점들은 `check` 와 6절 안내로 덮인다.
