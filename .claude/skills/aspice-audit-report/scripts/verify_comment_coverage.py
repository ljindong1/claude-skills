...
# --- Phase C(evidence 브리핑 게이트) ---
# Pass 인정에 필요한 '인용 근거' 패턴(정직한 한계 토큰은 제외 — Pass는 실제 근거가 있어야).
PASS_CITE_RE = re.compile(
    r"\.(docx|xlsx|xls|pdf|md)|절\s?[\'\d]|표\s?\d|시트|Rev|V\d|개정|이력|"
    r"회의록|검토결과서|서명|승인\s?기록|배포\s?기록|WBS|필드|No\.\s?\d")
# Not-Pass(확인 필요)에서 '읽어야 할 산출물/활동을 지목했는가' 판단 패턴
# (주의: '확인 필요' 자체는 지목이 아니므로 넣지 않는다)
POINTER_RE = re.compile(
    r"확인\s?요망|읽어|정독|산출물|계획서|명세서|설계서|사양서|요구사양|검토결과서|회의록|"
    r"codebeamer|PMS|레드마인|형상|시스템|절\s?[\'\d]|식별되지\s?않|식별\s?안|불명|폴더\s?외|접근|등록|배포|승인")
...
def main(result, human=None, only=None, require_intent=False):
    ...
    for kind in ("wp", "process"):
        for name in sh[kind]:
            ...
            for no, g, ap, cmt, res in rows(ws, kind):
                ...
                # --- Phase C: evidence 브리핑 게이트 ---
                pass_like = (res in config.PASS_FAMILY) or (exp == "Pass")
                if pass_like and not PASS_CITE_RE.search(ct):
                    scope_warn.append((no, gg[:18], "[Pass 근거 없음] Pass/Pass후보인데 인용 근거(파일·절·표·시트·버전·회의록·검토결과서 등)가 없음 — 인용 필수(comment_style §11)"))
                is_notpass = ((not pass_like) and (exp != "Fail") and ("Fail 후보" not in ct)
                              and ("[조건부 활동]" not in ct) and ("미대상" not in ct))
                if is_notpass and ct.strip() and not POINTER_RE.search(ct):
                    scope_warn.append((no, gg[:18], "[Not-Pass 지목 없음] 확인 필요인데 읽어야 할 산출물/활동 미지목 — 지목하거나 '식별되지 않음'을 명시(comment_style §12)"))
                if require_intent and ct.strip() and "[의도]" not in ct:
                    scope_warn.append((no, gg[:18], "[의도 라인 없음] 점검대상 코멘트 머리에 [의도] 필요(comment_style §10)"))
                d = by[g]
                d["t"] += 1
...
if __name__ == "__main__":
    ...
    ap.add_argument("--require-intent", action="store_true", help="점검대상 코멘트에 [의도] 라인 필수 검사(evidence 브리핑 모드)")
    a = ap.parse_args()
    sys.exit(main(a.result, a.human, a.sheet, a.require_intent))
