...
창은 뜨지 않는다 (`Visible = $false`). 다만 CANoe 프로세스 자체는 뜨므로,
CANoe 를 열어 두고 작업 중이면 닫고 돌린다.

**추출·대조(`verify`)는 CANoe 없이 된다.** 참조 무결성, DB 현행 여부,
CDD 가 물려 있는지는 전부 `.cfg` 를 파싱해서 나온다. `check` 만 CANoe 가
필요하고, 그것은 "CANoe 가 이 경로를 실제로 어떻게 해석하는가"를 묻는
명령이기 때문이다.
